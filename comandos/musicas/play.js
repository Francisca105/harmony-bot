...
exports.run = async (client, message, args, queue) => {
  ...
  if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
    ...
    return message.channel.send(`${p} A playlist \`${playlist.title}\` foi adicionada na fila de músicas!`).then(msg => msg.delete(8000));
  } else {
    try {
      ...
      try {
        ...
        try {
          ...
          const noPick = new Discord.RichEmbed()
            ...
            .setColor(client.c.corpadrão)
            .setTimestamp()
          message.channel.send({ embed: noPick }).then(msg => msg.delete(8000));
          msgtoDelete.delete()
          return;
        }
...
▫ Duração: **${song.durationh}:0${song.durationm}:0${durations}**`)
            .setFooter("© Harmony Network", client.user.avatarURL)
